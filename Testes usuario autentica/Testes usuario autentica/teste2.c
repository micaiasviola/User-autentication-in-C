...
{
    FILE *carregauser = fopen("idusersenha.txt", "r");

    if (carregarUsuarios == NULL)
    {
        perror("ERRO");
        return;
    }

    idUsuario = 0; // zero a quantidade de ids para um nova leitura

    while (fgets(user[idUsuario].nome, MAX_NOME, carregauser) != NULL)
    {
...
void cadastroUser()
{
    char buffer[100]; // variavel temporario para armazenar a linha lida

    Usuario novoUsuario; // Variavel temporaria para novo usuario

    // LER O NOME DO USUARIO-----------------------------------------------------------------------------------
    FILE *arquivo = fopen("idusersenha.txt", "r"); // Abertura do arquivo no modo de leitura
    ...
    {
        if (ch == '\n') // quando o ch encontrado for um \n ele vai adicionar uma linha amais aos contadores
        {
            idUsuario++;
            linhas++;
        }
    ...
    if (fgets(novoUsuario.nome, MAX_NOME, stdin) != NULL)
    {
        ...
        if (strlen(novoUsuario.nome) >= 4)
        {
            ...
            while (fgets(buffer, sizeof(buffer), arquivo) != NULL) // importante o !NULL faz o fgets retornar apenas quando chegar ao fim do arquivo
            {                                                      // verifica se o usuario ja existe
                ...
                if (parse != NULL)
                {
                    ...
                    if (parse != NULL)
                    {
                        if (strcmp(parse, novoUsuario.nome) == 0)
                        {
                            usuarioExistente = 1;
                            break;
                        }
            ...
            else
            {
                ...
                fprintf(arquivoescrita, "%d|%s\n", idUsuario + 1, user[idUsuario].nome);
