...
/***VARIAVEIS GLOBAIS****/
int idUsuario = 0; // inicia o id sempre em 0 para come�ar a varredura
int ident = 0;
int excedido = 0;
char senhaADM[] = "1234";
...
void limpabuffer() // fun��o para limpar o buffer apos um endere�o de memoria ser escrito, chamo ap�s os fgets
{
    int c;
    while ((c = getchar()) != '\n' && c != EOF)
    {
        // corpo vazio para limpar o endere�o de memoria
    }
}

void lerUsuarios() // fun��o que executa a leitura do banco de dados, armazena a quantidade de cadastros em idUsuario
{
    ...
    while (fgets(user[idUsuario].nome, MAX_NOME, leitura) != NULL)
    {
        ...
        char *split = strtok(user[idUsuario].nome, "|"); // primeiro token a ser pego da tabela � o id, portanto split esta valendo o id no momento
        ...
        if (split != NULL)
        {
            ...
            split = strtok(NULL, "|"); // agora split recebe o nome, pois foi chamado a fun��o null dele, ent�o ele entende como 1token � nulo e apos a divis�o ele ira armazenar o 2token que no caso � o nome
            strcpy(user[idUsuario].nome, split);
            // printf("%s\n", user[idUsuario].nome);
            if (split != NULL)
            {
                split = strtok(NULL, "|"); // aqui o split ira ignorar o token atual (2) e ler a partir da proxima divis�o, que no caso sera o 3token == senha
...
void cadastrarUsuario() // fun��o responsavel pelo cadastro de usuarios, verifica as condi��es para cadastro
{
    if (excedido == 1)
    {
    ...
    if (fgets(senhaAdmin, MAX_SENHA, stdin) != NULL) // insere senha de adm
    {
        ...
        limpar_tela();
        if (strcmp(senhaAdmin, senhaADM) == 0) // compara senha de adm para acessar a fun��o de cadastrar
        {
            estrutura novoUsuario; // preciso de uma variavel do tipo struct igual ao ser para fazer a compara��o dos dados inseridos e ver se o usuario existe ou n�o
            ...
            {
                novoUsuario.nome[strcspn(novoUsuario.nome, "\n")] = '\0'; // strcspn procura pela posi��o da string em que o \n se encontra e o substitui por \0

                if (strlen(novoUsuario.nome) >= 4) // strlen() tamanho da variavel
                {
                    int usuarioExiste = 0;
                    for (int i = 0; i < idUsuario; i++) // esse looping ira percorrer a quantidade de vezes e linhas que existem usuarios atraves da leitura feita na fun��o lerusuarios armazenada em idUsuario
                    {
                        if (strcmp(user[i].nome, novoUsuario.nome) == 0) // se linha atual == novo nome ent�o usuario existe
                        {
                            usuarioExiste = 1;
                            break;
                        }
                    }
                    if (usuarioExiste) // condi��o de controle para usuario existente
                    {
                        limpar_tela();
                        printf("Usuario existente\n");
                    }
                    else // caso o usuario n�o existe ele ira para essa condi��o, no caso aqui sera feito o cadastro
                    {
                        printf("\tDigite a senha: ");
                        if (fgets(novoUsuario.senha, MAX_SENHA, stdin) != NULL) // endere�a valor de senha
                        {
                            ...
                            if (strlen(novoUsuario.senha) >= 4)                         // se senha for maior ou igual a 4 caracteres
                            {
                                ...
                                {
                                    limpabuffer(); // Limpar buffer ap�s scanf
                                    ...
                                    {
                                        limpar_tela();
                                        printf("Op��o inv�lida. Usu�rio n�o cadastrado.\n");
                                        return;
                                    }

                                    user[idUsuario] = novoUsuario; // endere�a os valores .nome, .senha
                                    idUsuario++;
                                ...
                                {
                                    limpar_tela();
                                    printf("Erro ao ler a op��o de cargo\n");
                                    limpabuffer(); // Limpar buffer ap�s scanf
                                    return;
                                }
                            }
...
int main()
{
    ...
    do
    {
        ...
        case 4:
            ...
            break;
        default:
            // N�o deve chegar aqui devido ao controle na fun��o obterOpcao()
            break;
        }
    } while (opcao != 3);
